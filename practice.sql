...
select d.departmentID,count(e.Employeeid) as no_of_employee from Employee E, department D 
WHERE E.DepartmentID=D.departmentID and employeeid is null GROUP BY DepartmentID; 
...
select Employeeid,name from employee e where exists (select * FROM DEPARTMENT D 
WHERE E.DEPARTMENTID=D.DEPARTMENTID and location = "Delhi" and "Mumbai"); 
select * from employee where departmentID in (select d.DepartmentID from department as d where location in ("Delhi" and "Mumbai"));
select * from employee where departmentID = (select departmentid from department where location in ("Delhi" and "Mumbai"));
select * from department;
select DepartmentID from department where location = "Delhi" and "Mumbai";
SELECT * FROM EMPLOYEE WHERE DEPARTMENTid IN (SELECT DEPARTMENTID FROM DEPARTMENT WHERE location in ("Delhi" and "Mumbai"));
...
(select d.departmentID,count(e.Employeeid) as one_man_army from Employee E, department D 
WHERE E.DepartmentID=D.departmentID and employeeid =1 GROUP BY DepartmentID); 
desc department;
DESC EMPLOYEE;
...
SELECT departmentid, SUM(salary) AS total_salary FROM employee
WHERE departmentid IN (SELECT departmentid FROM employee GROUP BY departmentid HAVING AVG(salary) > 55000)
